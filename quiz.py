...
def quiz_form():
    ...
    <html>
        <head>
            <title>Приветствие и Выбор Викторины</title>
        </head>
        <body>
            <h2>Привет, мой дорогой друг! Я рад приветствовать тебя на моём сайте викторин. На нём ты можешь выбрать викторину по теме из 3 предложенных школьных дисциплин, пройти её и, таким образом, проверить, насколько обширны твои знания в данной теме. Для тебя я подготовил викторины на следующие темы: География, Математика и Русский язык. В каждой из них представлено 9 вопросов, в каждом из которых 4 варианта ответа, и только один верный. Помни, что всё на свете знать абсолютно невозможно, поэтому у каждого из нас есть право ошибиться. По окончанию викторины выведится твой результат.</h2>
            <h3>Итак, предлагаю тебе выбрать викторину на одну из 3 предложенных ниже в списке тем:</h3>
...
def result():
    ...
    <html>
        <head>
            <title>Поздравление и Результат Прохождения Викторины</title>
        </head>   
        <body>
            <h2>Поздравляю! Ты закончил прохождение викторины. Итак, Вот твой результат: Правильных ответов:</h2>
        </body>
    </html>
    '''
    result = str(session.get('answers', 0)) + " из " + str(session.get('total', 0))
    return html + result
